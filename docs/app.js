(() => {
  ...
  function renderMetrics() {
    ...
    $("#metrics").innerHTML = [
      ...
      [counts.reason_assertions, "reasons"],
      [counts.compounds, "compounds"],
      [counts.outcome_comparisons, "outcomes"],
  ...
  function filteredMoves() {
    ...
    return moves.filter((move) => {
      ...
      const haystack = [reason.source_title, reason.reason_statement, reason.child_paper_label, reason.intended_property.name, reason.evidence.document_id].join(" ").toLowerCase();
  ...
  function renderList() {
    ...
    $("#episode-list").innerHTML = visible.length ? visible.map((move) => {
      ...
      return `<button class="episode-card ${move.reasoned_move_id === state.selected ? "active" : ""}" data-id="${escapeHtml(move.reasoned_move_id)}">
        <span class="card-meta"><span>${escapeHtml(reason.evidence.document_id)}</span><span>${reason.evidence.publication_year ?? ""}</span></span>
        <h3>${escapeHtml(reason.child_paper_label)} · ${escapeHtml(humanize(reason.intended_property.name))}</h3>
  ...
  function deltaText(row) {
    ...
    if (row.delta_lower != null && row.delta_upper == null) return `≥ ${row.delta_lower}`;
    if (row.delta_lower == null && row.delta_upper != null) return `≤ ${row.delta_upper}`;
    return `${row.delta_lower}`;
  }
  ...
  function renderDetail() {
    ...
    $("#detail").innerHTML = `
      <div class="detail-header">
        <div>
          <p class="detail-kicker">${escapeHtml(reason.evidence.document_id)} · ${reason.evidence.publication_year ?? ""} · child ${escapeHtml(reason.child_paper_label)}</p>
          <h2>${escapeHtml(reason.source_title ?? reason.evidence.document_id)}</h2>
        </div>
        <a class="evidence-link" href="${escapeHtml(reason.evidence.citation_url)}">Evidence lines ↗</a>
      </div>
      ...
      <div class="flow">
        ...
        ${moleculeCard(child, "Reason-bearing child")}
      </div>
      ...
      <section class="outcomes">
        ...
        <div class="table-wrap"><table>
          <thead><tr><th>Endpoint / state</th><th>Comparator</th><th>Child</th><th>Result</th><th>Intent relationship</th></tr></thead>
          <tbody>${outcomes.map((row) => `<tr>
            ...
            <td class="measurement">${formatMeasurement(row.parent_relation, row.parent_value, row.units)}</td>
            <td class="measurement">${formatMeasurement(row.child_relation, row.child_value, row.units)}</td>
